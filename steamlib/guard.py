...
class SteamGuard:
    ...
    def finalize(self, activation_code):
        ...
        response = self._session.post(f'{APIEndpoint.TWO_FACTOR_URL}FinalizeAddAuthenticator/v1/', data=data)
        print(response)
        print(response.json())
    ...
    def remove(self, revocation_code):
        ...
        response = self._session.post(f'{APIEndpoint.TWO_FACTOR_URL}RemoveAuthenticator/v1/', data=data)
    ...
    def get_code(self, timestamp=None):
        ...
        return self._generate_twofactor_code(b64decode(shared_secret), self._get_time() if timestamp is None else timestamp)
    ...
    def _get_time(self):
        if (self._steam_time_offset is None or (self._align_time_every and (time() - self._offset_last_check) > self._align_time_every)):
    ...
    def _get_time_offset(self):
        try:
            ...
            response = self._session.post(f'{APIEndpoint.TWO_FACTOR_URL}QueryTime/v1/', params=params).json()
        except:
            return None
        time_now = int(time())
