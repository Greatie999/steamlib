...
class LoginExecutor:
    ...
    def _send_login(self) -> requests.Response:
        ...
        resp = self._session.post(
            f'{APIEndpoint.COMMUNITY_URL}login/dologin/', 
            data=data
        ).json()
    ...
    def _set_session_id(self) -> None:
        ...
        community_cookie = self._create_session_id_cookie(
            sessionid, 
            community_domain
        )
    ...
    def _captcha_url(self) -> str:
        if self._captcha_gid == -1:
            return None
        return f'{APIEndpoint.COMMUNITY_URL}login/rendercaptcha/?gid={self._captcha_gid}'
...
class MobileLoginExecutor(LoginExecutor):
    ...
    def oauth_login(self) -> requests.Session:
        ...
        self._session_id = self._session.get(APIEndpoint.COMMUNITY_URL).cookies.get_dict()['sessionid']
